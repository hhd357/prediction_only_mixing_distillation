...
model.fc = nn.Identity()   # 512-dim features (same as ResNet-18)
...
# Fixed splits (seeds 0, 1, 2)
rng_train = np.random.default_rng(0)
rng_val   = np.random.default_rng(1)
rng_test  = np.random.default_rng(2)

# Train + fresh from train_dataset_full
train_indices     = rng_train.choice(len(train_dataset_full), n_train, replace=False)
remaining_indices = np.setdiff1d(np.arange(len(train_dataset_full)), train_indices)
fresh_indices     = rng_train.choice(remaining_indices, n_fresh, replace=False)

# Val from test_dataset_full
all_test_indices = rng_val.permutation(len(test_dataset_full))
val_indices      = all_test_indices[:n_val]

# Test from test_dataset_full
remaining_test_indices = np.setdiff1d(np.arange(len(test_dataset_full)), val_indices)
test_indices           = rng_test.choice(remaining_test_indices, n_test, replace=False)
...
print(f"  X_fresh : {X_fresh.shape}, y_fresh : {y_fresh.shape}")
...
y_train_multi = encoder.fit_transform(y_train.reshape(-1, 1))
...
FINE_TO_SUPER = np.zeros(100, dtype=int)
for sup, fines in CIFAR100_SUPERCLASS_MAP.items():
    for f in fines:
        FINE_TO_SUPER[f] = sup
...
for fine in range(K):
    ...
    same_super_set         = CIFAR100_SUPERCLASS_MAP[sup]
    same_super_fines[fine] = np.array([f for f in same_super_set if f != fine])

# Within-superclass corruption
def corrupt_labels_superclass(y, K, same_super_fines, corruption_rate, rng):
    """
    Corrupt a fraction of fine-grained labels by replacing each with a
    uniformly random fine class from the SAME superclass (excluding itself).
    """
    y_corrupted = y.copy()
    n           = len(y)
    ...
    for idx in corrupt_idx:
        y_corrupted[idx] = rng.choice(same_super_fines[y[idx]])
    corrupted_mask              = np.zeros(n, dtype=bool)
    corrupted_mask[corrupt_idx] = True
    return y_corrupted, corrupted_mask

# Each split gets its own fixed rng for corruption (matching split seeds 0/1/2)
y_train_corrupted, train_corrupted_mask = corrupt_labels_superclass(
    y_train, K, same_super_fines, noise_rate, np.random.default_rng(0))
y_val_corrupted,   val_corrupted_mask   = corrupt_labels_superclass(
    y_val,   K, same_super_fines, noise_rate, np.random.default_rng(1))
y_test_corrupted,  test_corrupted_mask  = corrupt_labels_superclass(
    y_test,  K, same_super_fines, noise_rate, np.random.default_rng(2))
...
# Reassign y_train to corrupted (val/test stay clean for evaluation)
...
# ── Summary ───────────────────────────────────────────────────────────────────
print(f"\nSplit summary:")
print(f"  X_train : {X_train.shape}  (noise_rate={noise_rate:.0%}, within-superclass)")
...
ax.set_title(rf'CIFAR-100 ($p={noise_rate:.1f}$)', fontsize=ftsize + 4)
