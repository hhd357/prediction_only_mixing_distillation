import numpy as np
import matplotlib.pyplot as plt
import matplotlib.cm as cm
from matplotlib.lines import Line2D
import pandas as pd

Array = np.ndarray

# Parameters
n_1 = 400 #labeled data
n_2 = 400 #unlabeled data
p = 200
sigma = 1.0
num_seeds = 30
start_seed = 2026

lambda_regs = np.logspace(np.log10(1e-2), np.log10(1e2), 40)
sigma_beta_type = 'top_aligned'

gamma_l = p / n_1
gamma_u = p / n_2

# SNR pairs: alpha = r, sigma = 1, SNR = r^2 / sigma^2 = alpha^2
pairs = [
    (np.sqrt(0.2), 1.0),
    (np.sqrt(0.5), 1.0),
    (1.0,          1.0),
    (np.sqrt(3),   1.0),
    (np.sqrt(5),   1.0),
]
num_pairs = len(pairs)

# Covariance setup
def ar1_covariance(p, rho=0.25):
    Sigma = np.zeros((p, p))
    for i in range(p):
        for j in range(p):
            Sigma[i, j] = rho ** abs(i - j)
    return Sigma

def spiked_covariance(p, num_spikes=1, spike_strength=5.0):
    Sigma = np.eye(p)
    for _ in range(num_spikes):
        v = np.random.randn(p)
        v /= np.linalg.norm(v)
        Sigma += spike_strength * np.outer(v, v)
    return Sigma

def random_wishart(p, m):
    X = np.random.randn(m, p)
    return X.T @ X / m

# Data covariance
# Sigma_1 = np.eye(p)
Sigma_1 = ar1_covariance(p)
Sigma_2 = Sigma_1
# Sigma_2 = 5 * Sigma_1 + 10*np.eye(p)

# For "top_aligned" or "bottom_aligned": use spike_frac and align_frac below
spike_frac = 0.1              # Sparse level: fraction of dimensions for top/bottom (e.g., 0.1 -> 10% of p)
align_frac = 0.9              # Alignment factor: fraction of total variance in aligned subspace (e.g., 0.9 -> 90% in top/bottom)

def generate_sigma_beta(V, p, sigma_beta_type, k_top=50, k_bottom=50, m_amp=20, amp_factor=10.0):
    if sigma_beta_type == "top50":
        D = np.ones(p) * 0.05
        D[-k_top:] = 1.0
        Sigma_beta = V @ np.diag(D) @ V.T
        trace = np.trace(Sigma_beta)
        Sigma_beta = (p / trace) * Sigma_beta
    elif sigma_beta_type == "bottom50":
        D = np.ones(p) * 0.05
        D[:k_bottom] = 1.0
        Sigma_beta = V @ np.diag(D) @ V.T
        trace = np.trace(Sigma_beta)
        Sigma_beta = (p / trace) * Sigma_beta
    elif sigma_beta_type == "mixed20":
        D = np.ones(p)
        D[:m_amp] *= amp_factor
        Sigma_beta = np.diag(D)
        trace = np.trace(Sigma_beta)
        Sigma_beta = (p / trace) * Sigma_beta
    elif sigma_beta_type == "identity":
        Sigma_beta = np.eye(p)
    elif sigma_beta_type == "top_aligned":
        k = int(spike_frac * p)
        D = np.zeros(p)
        D[-k:] = align_frac * p / k      # Top k: equal share of aligned energy
        D[:-k] = (1 - align_frac) * p / (p - k)  # Bottom: equal share of unaligned
        Sigma_beta = V @ np.diag(D) @ V.T
    elif sigma_beta_type == "bottom_aligned":
        k = int(spike_frac * p)
        D = np.zeros(p)
        D[:k] = align_frac * p / k        # Bottom k: equal share of aligned energy
        D[k:] = (1 - align_frac) * p / (p - k)  # Top: equal share of unaligned
        Sigma_beta = V @ np.diag(D) @ V.T
    else:
        raise ValueError(f"Invalid sigma_beta_type: {sigma_beta_type}")
    return Sigma_beta


def excess_risk(beta_hat, beta_star, Sigma, sigma_noise):
    diff = beta_hat - beta_star
    return diff @ Sigma @ diff + sigma_noise**2

# Theory: solve kappa fixed-point
def solve_kappa_bisect(eigs: Array, gamma: float, lam: float,
                       tol: float = 1e-12, maxit: int = 400) -> float:
    s = np.asarray(eigs, dtype=float)

    def f(k):
        return k - lam - gamma * k * np.mean(s / (s + k))

    lo = 0.0
    hi = max(lam + gamma * float(s.max()) + 1.0, 1.0)
    while f(hi) <= 0.0:
        hi *= 2.0
        if hi > 1e12:
            raise RuntimeError("Failed to bracket the kappa fixed point.")
    for _ in range(maxit):
        mid = 0.5 * (lo + hi)
        if f(mid) > 0.0:
            hi = mid
        else:
            lo = mid
        if hi - lo < tol * max(1.0, hi):
            break
    return 0.5 * (lo + hi)

# Theory: fresh-X deterministic equivalents
def theory_fresh_full_general(Sigma: Array, beta: Array, sigma2: float,
                               gamma_l: float, gamma_u: float,
                               lambdas: Array) -> pd.DataFrame:
    eigs, U = np.linalg.eigh(Sigma)
    beta_e = U.T @ beta

    rows = []
    for lam in np.asarray(lambdas, dtype=float):
        kappa   = solve_kappa_bisect(eigs, gamma_l, float(lam))
        kappa_u = solve_kappa_bisect(eigs, gamma_u, float(lam))

        g   = 1.0 / (eigs + kappa)
        g_u = 1.0 / (eigs + kappa_u)

        t2   = gamma_l * np.mean((eigs ** 2) * (g ** 2))
        b    = 1.0 / (1.0 - t2)
        u2   = t2 * b

        t2u_unlab = gamma_u * np.mean((eigs ** 2) * (g_u ** 2))
        b_u  = 1.0 / (1.0 - t2u_unlab)

        q20 = np.sum((beta_e ** 2) * eigs * (g ** 2))
        q11 = np.sum((beta_e ** 2) * eigs * g * g_u)
        q21 = np.sum((beta_e ** 2) * eigs * (g ** 2) * g_u)
        q12 = np.sum((beta_e ** 2) * eigs * g * (g_u ** 2))
        q22 = np.sum((beta_e ** 2) * eigs * (g ** 2) * (g_u ** 2))
        q02 = np.sum((beta_e ** 2) * eigs * (g_u ** 2))

        t21 = gamma_l * np.mean((eigs ** 2) * (g ** 2) * g_u)
        t22 = gamma_l * np.mean((eigs ** 2) * (g ** 2) * (g_u ** 2))

        bar_R     = (kappa ** 2) * b * q20 + sigma2 * u2
        bar_Delta = kappa_u * (
            -kappa * q11
            + (kappa ** 2) * q21
            + b * t21 * ((kappa ** 2) * q20 + sigma2)
        )
        bar_D = (kappa_u ** 2) * b_u * (
            q02
            - 2.0 * kappa * q12
            + (kappa ** 2) * q22
            + b * t22 * ((kappa ** 2) * q20 + sigma2)
        )
        bar_C    = bar_R - bar_Delta
        bar_Rpd  = bar_R - 2.0 * bar_Delta + bar_D

        xi      = bar_Delta / bar_D
        R_te    = sigma2 + bar_R
        C_fresh = sigma2 + bar_C
        R_pd    = sigma2 + bar_Rpd
        R_pm    = R_te - (R_te - C_fresh)**2 / (R_te + R_pd - 2 * C_fresh)

        rows.append({
            'lambda':          lam,
            'theory_R':        R_te,
            'theory_C':        C_fresh,
            'theory_D':        bar_D,
            'theory_Rpd':      R_pd,
            'theory_xi_star':  xi,
            'theory_Rpm_star': R_pm,
            'theory_Delta':    bar_Delta,
            'theory_A':        bar_R,
            'theory_B':        bar_Rpd,
            'theory_C_cross':  bar_C,
        })
    return pd.DataFrame(rows)

# Main loop: sweep over SNR pairs
full_results = {}

np.random.seed(start_seed - 1)
eigvals, V = np.linalg.eigh(Sigma_1)
Sigma_beta = generate_sigma_beta(V, p, sigma_beta_type)

for (alpha_val, sigma_val) in pairs:
    snr = alpha_val**2 / sigma_val**2
    print(f"\n=== SNR = {snr:.2f} (alpha={alpha_val:.4f}, sigma={sigma_val}) ===")

    # Fix beta_star for this SNR
    np.random.seed(start_seed - 1)
    cov_beta  = (alpha_val**2 / p) * Sigma_beta
    beta_star = np.random.multivariate_normal(np.zeros(p), cov_beta)

    collected_risk_pm  = []
    collected_xi_pm    = []

    for current_seed in range(start_seed, start_seed + num_seeds):
        print(f"  Seed {current_seed}", end='\r')
        np.random.seed(current_seed)

        X_train = np.random.multivariate_normal(np.zeros(p), Sigma_1, size=n_1)
        noise   = np.random.randn(n_1) * sigma_val
        y_train = X_train @ beta_star + noise

        X_fresh = np.random.multivariate_normal(np.zeros(p), Sigma_2, size=n_2)

        I_p        = np.eye(p)
        S_train    = X_train.T @ X_train / n_1
        S_fresh    = X_fresh.T @ X_fresh / n_2

        seed_r_pm  = []
        seed_xi_pm = []

        for lambda_reg in lambda_regs:
            # Teacher
            Omega    = S_train + lambda_reg * I_p
            beta_hat = np.linalg.solve(Omega, X_train.T @ y_train) / n_1

            # PD student
            Omega_fresh = S_fresh + lambda_reg * I_p
            M_fresh     = np.linalg.solve(Omega_fresh, S_fresh)
            beta_pd     = M_fresh @ beta_hat

            # Oracle PM
            error       = beta_hat - beta_star
            tilde_error = beta_pd  - beta_star
            A = error       @ Sigma_1 @ error
            B = tilde_error @ Sigma_1 @ tilde_error
            C = error       @ Sigma_1 @ tilde_error

            xi      = (A - C) / (A + B - 2 * C)
            beta_pm = (1 - xi) * beta_hat + xi * beta_pd
            seed_xi_pm.append(xi)
            seed_r_pm.append(excess_risk(beta_pm, beta_star, Sigma_1, sigma_val))

        collected_risk_pm.append(seed_r_pm)
        collected_xi_pm.append(seed_xi_pm)

    # Average
    emp_pm_means  = np.mean(collected_risk_pm, axis=0)
    emp_pm_sems   = np.std(collected_risk_pm,  axis=0) / np.sqrt(num_seeds)
    emp_xi_means  = np.mean(collected_xi_pm,   axis=0)
    emp_xi_sems   = np.std(collected_xi_pm,    axis=0) / np.sqrt(num_seeds)

    # Theory
    theory_df   = theory_fresh_full_general(
        Sigma=Sigma_1, beta=beta_star, sigma2=sigma_val**2,
        gamma_l=gamma_l, gamma_u=gamma_u, lambdas=lambda_regs,
    )
    asym_pm   = theory_df['theory_Rpm_star'].values
    asym_xi   = theory_df['theory_xi_star'].values

    full_results[(alpha_val, sigma_val)] = {
        'emp_pm_means': emp_pm_means,
        'emp_pm_sems':  emp_pm_sems,
        'emp_xi_means': emp_xi_means,
        'emp_xi_sems':  emp_xi_sems,
        'asym_pm':      asym_pm,
        'asym_xi':      asym_xi,
    }


### Plotting
plt.rcParams.update({
    'text.usetex': False,
    'font.family': 'sans-serif',
})

fig, axes = plt.subplots(1, 2, figsize=(12, 5), sharex=True, sharey=False)

cmap   = cm.get_cmap('RdPu')
colors = [cmap(0.40 + 0.65 * i / (num_pairs - 1)) for i in range(num_pairs)] \
         if num_pairs > 1 else [cmap(0.5)]

ftsize = 14

# R_pm
ax_left = axes[0]
for i, pair in enumerate(pairs):
    alpha_val, sigma_val = pair
    snr   = alpha_val**2 / sigma_val**2
    d     = full_results[pair]
    color = colors[i]

    lower = d['emp_pm_means'] - d['emp_pm_sems']
    upper = d['emp_pm_means'] + d['emp_pm_sems']
    ax_left.fill_between(lambda_regs, lower, upper, alpha=0.2, color=color)
    ax_left.plot(lambda_regs, d['emp_pm_means'], '^--',
                 color=color, linewidth=2, alpha=0.8, markevery=4)  # empirical: dashed + triangle
    ax_left.plot(lambda_regs, d['asym_pm'], '-',
                 color=color, linewidth=2)                          # theoretical: solid

ax_left.set_xscale('log')
ax_left.set_xlabel(r'Ridge penalty $\lambda$', fontsize=ftsize + 2)
ax_left.set_ylabel('Squared prediction risk', fontsize=ftsize + 2)
ax_left.set_title(r'Optimal PM risk $R_{\mathrm{pm}}^{\star}$', fontsize=ftsize + 2)
ax_left.grid(True, alpha=0.3)

ax_left.tick_params(axis='x', labelsize=ftsize)
ax_left.tick_params(axis='y', labelsize=ftsize)


# xi
ax_right = axes[1]
for i, pair in enumerate(pairs):
    alpha_val, sigma_val = pair
    d     = full_results[pair]
    color = colors[i]

    lower = d['emp_xi_means'] - d['emp_xi_sems']
    upper = d['emp_xi_means'] + d['emp_xi_sems']
    ax_right.fill_between(lambda_regs, lower, upper, alpha=0.2, color=color)
    ax_right.plot(lambda_regs, d['emp_xi_means'], '^--',
                  color=color, linewidth=2, alpha=0.8, markevery=4)  # empirical: dashed + triangle
    ax_right.plot(lambda_regs, d['asym_xi'], '-',
                  color=color, linewidth=2)                           # theoretical: solid

ax_right.set_xscale('log')
ax_right.set_xlabel(r'Ridge penalty $\lambda$', fontsize=ftsize + 2)
ax_right.set_ylabel(r'$\xi^{\star}$', fontsize=ftsize + 2)
ax_right.set_title(r'Optimal mixing parameter $\xi^{\star}$', fontsize=ftsize + 2)
ax_right.grid(True, alpha=0.3)
ax_right.tick_params(axis='x', labelsize=ftsize)
ax_right.tick_params(axis='y', labelsize=ftsize)

# Shared legend 
type_handles = [
    Line2D([0], [0], color='gray', lw=2, linestyle='-',
           label='Theoretical'),
    Line2D([0], [0], color='gray', lw=2, marker='^', markersize=6,
           linestyle='--', label='Empirical', markevery=3),
]

snr_handles = []
snr_labels  = []
for i, (alpha_val, sigma_val) in enumerate(pairs):
    snr = alpha_val**2 / sigma_val**2
    snr_handles.append(Line2D([0], [0], color=colors[i], lw=2,
                               label=f'SNR={snr:.1f}'))
    snr_labels.append(f'SNR={snr:.1f}')

all_handles = type_handles + snr_handles
all_labels  = [h.get_label() for h in type_handles] + snr_labels

ax_left.set_xticks([])
ax_left.set_xticks([], minor=True)
ticks = [0.01, 0.1, 1, 10, 100]
ax_left.set_xticks(ticks)
ax_left.set_xticklabels(['0.01', '0.1', '1', '10', '100'])

ax_left.set_yticks([])
ax_left.set_yticks([], minor=True)
ticks_y = [1, 1.5, 2, 2.5]
ax_left.set_yticks(ticks_y)
ax_left.set_yticklabels(['1', '1.5', '2', '2.5'])

fig.legend(handles=all_handles, labels=all_labels,
           loc='upper center', bbox_to_anchor=(0.5, -0.05),
           ncol=4, frameon=False, fontsize=ftsize)

fig.tight_layout()
fig.subplots_adjust(bottom=0.05, wspace=0.4)

plt.savefig(
    "snr_pm_xi_plot.pdf",
    format="pdf",
    bbox_inches='tight',
    edgecolor='none',
    transparent=True,
)
plt.show()